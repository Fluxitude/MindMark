// MindMark Design System - FamilyButton Component
// Pixel-perfect replication of original Cult UI FamilyButton
...
// Main component props - simplified to match original
export interface FamilyButtonProps {
  children: React.ReactNode;
  className?: string;
}

// Original Cult UI FamilyButton implementation
const FamilyButton: React.FC<FamilyButtonProps> = ({ children, className }) => {
...
// Container component that handles animations
...
const FamilyButtonContainer: React.FC<FamilyButtonContainerProps> = ({
  isExpanded,
  toggleExpand,
  children,
}) => {
    ...
    >
      ...
      >
        {isExpanded ? (
          ...
          >
            <Plus className="h-7 w-7 text-black dark:text-neutral-900" />
          </motion.div>
        )}
      </motion.div>
    </motion.div>
...
export interface FamilyButtonHeaderProps {
  children: React.ReactNode;
  className?: string;
}

export const FamilyButtonHeader: React.FC<FamilyButtonHeaderProps> = ({
  children,
  className
}) => (
  <div className={cn("flex flex-col gap-4 justify-center items-center", className)}>
    {children}
  </div>
);

export interface FamilyButtonContentProps {
  children: React.ReactNode;
  className?: string;
}

export const FamilyButtonContent: React.FC<FamilyButtonContentProps> = ({
  children,
  className
}) => (
